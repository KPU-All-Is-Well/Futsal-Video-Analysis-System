...
if __name__ == '__main__':
    ...
    for player in range(1, total_player+1): # 선수 1부터 total_player까지 반복문
        ...
        videoPath = 'kpufutsal_walk.mp4'   # 비디오를 읽어옴
            ...
            for i, newbox in enumerate(boxes):
                ...
                if (i<6):
                    ...
                    if((frame_cnt%fps)==0) :     # 프레임기반 1초(fps)마다 동작하는 코드
                        ...
                        a = (int)(((newbox[0]) - lastPoint.x) * 3.068)
                        ...
                        distance = estimate_distance / 100      # 추정거리에서 도출된 값에 가중치를 두고 m단위로 변환 
                        distance = round(distance,2)                # 반올림 처리           
                        speed = (moving_weight / 100)*3.6       # moving_weight를 통해 구해진 m/s에 3.6을 곱해 속도를 k/h로 바꿔줌
                        speed = round(speed,2)
